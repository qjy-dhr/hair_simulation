...
public:
    Viewer(std::string title, bool (*pre_draw_callback)(Viewer*) = nullptr, bool (*mesh_load_callback)(Viewer*) = nullptr)
        ...
        //255, 255, 240, 1
        setBackground(nanogui::Color(245, 245, 245, 1));
        initGUI();
        initShaders();
    ...
    void defaultRods(const size_t res = 10, const size_t num_rods = 1) {
    ...
    void addRodsOnBall(float ball_radius, size_t res = 5, size_t num_rods = 5, float seg_length = 0.1, float hair_thickness = 0.005) {
        using_rods = true;
        r_res = res;
        r_num_rods = num_rods;
        size_t nPoints = res*num_rods;
        m_thickness = hair_thickness;
        m_updated_rods_pos.resize(3, nPoints);
        m_updated_rods_tangents.resize(3, nPoints);
        m_updated_rods_normals.resize(3, nPoints);
        m_rod_indices.clear();
        ...
        meshProcess();
        m_mesh_load_callback(this); //将位置重新上传到模拟器，因为我们更改了它们
    }
	...
	void rodMesh() {
        ...
        for (size_t ind = 0; ind < m_rod_face_indices.size()-1; ind++) {
            ...
            for (size_t i = 0; i < (r_faces/r_num_rods-ndivs)/2; i++) {
                ...
                v = i == 0 || (i+1) % ndivs != 0 ? v_m.at(rod_index+i+1) : v_m.at(rod_index+i-(ndivs-1));
                w = v_m.at(rod_index+i+ndivs);
                x = i == 0 || (i+1) % ndivs != 0 ? v_m.at(rod_index+i+ndivs+1) : v_m.at(rod_index+i+1);
    ...
    void meshProcess() {
        ...
        mShader.setUniform("intensity", Vector3f(0.99, 1.0, 1.0));
    ...
    void rodProcess() {
        ...
        rShader.setUniform("intensity", Vector3f(0.21, 0.39, 0.80));
    ...
    void initGUI() {
        ...
        //ENTYPO_ICON_EXPORT
        PopupButton* popupBtn = new PopupButton(window, "Open a mesh");
    ...
    virtual void drawContents() {
        ...
        //Vector3f colors(1.21, 1.63, 1.29);偏黄色
        Vector3f colors(1.33, 2.0, 1.77);
